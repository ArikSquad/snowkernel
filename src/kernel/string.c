...
void *kmemset(void *d, int v, size_t n)
{
	unsigned char *p = d;
	for (size_t i = 0; i < n; ++i)
		p[i] = (unsigned char)v;
...
static char _kdup_buf[4096];
static size_t _kdup_off = 0;
...
char *kstrdup(const char *s)
{
	...
	if (n > sizeof(_kdup_buf))
		return 0;
	if (_kdup_off + n > sizeof(_kdup_buf))
	{
		_kdup_off = 0;
	}
	char *p = &_kdup_buf[_kdup_off];
	for (size_t i = 0; i < n; ++i)
		p[i] = s[i];
	_kdup_off += n;
	return p;
